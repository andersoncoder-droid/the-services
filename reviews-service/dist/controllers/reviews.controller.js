...
import { createReviewSchema, updateReviewSchema, reviewIdSchema, reviewsQuerySchema, productIdSchema, } from "../schemas/review.schema.js";
...
export async function createReview(req, res) {
    try {
        ...
        const [existingReview] = await db.query("SELECT calificacion_id FROM calificaciones WHERE producto_id = ? AND usuario_id = ?", [validatedData.producto_id, usuario_id]);
        if (existingReview.length > 0) {
            return res.status(409).json({
                message: "Ya has reseñado este producto. Puedes actualizar tu reseña existente.",
            });
        }
        // Crear la reseña
        const [result] = await db.query("INSERT INTO calificaciones (producto_id, usuario_id, calificacion, comentario, fecha) VALUES (?, ?, ?, ?, NOW())", [
      ...
      WHERE c.calificacion_id = ?`, [result.insertId]);
        res.status(201).json({
...
export async function getReviews(req, res) {
    try {
        ...
        const { page, limit, producto_id, usuario_id, sort, order, calificacion_min, calificacion_max, } = validatedQuery;
    ...
    `;
        ...
        // Ejecutar consulta
        const [reviews] = await db.query(baseQuery, queryParams);
        const response = {
            total,
            pages,
            first,
...
export async function updateReview(req, res) {
    try {
        ...
        const [existingReview] = await db.query("SELECT usuario_id FROM calificaciones WHERE calificacion_id = ?", [id]);
        if (existingReview.length === 0) {
            return res.status(404).json({ message: "Reseña no encontrada" });
        }
        ...
        await db.query(`UPDATE calificaciones SET ${updateFields.join(", ")} WHERE calificacion_id = ?`, updateValues);
        // Obtener la reseña actualizada
        const [updatedReview] = await db.query(`SELECT 
        c.calificacion_id,
        c.producto_id,
        c.usuario_id,
        'Usuario' as nombre_usuario,
...
export async function deleteReview(req, res) {
    try {
        ...
        // Eliminar la reseña
        await db.query("DELETE FROM calificaciones WHERE calificacion_id = ?", [id]);
...
export async function getReviewsByProduct(req, res) {
    ...
    `;
        ...
        // Obtener total de registros
        const [totalResult] = await db.query("SELECT COUNT(*) as total FROM calificaciones WHERE producto_id = ?", [producto_id]);
        const total = totalResult[0].total;
        // Calcular paginación
        const pages = Math.ceil(total / limit);
        const currentPage = Math.max(1, Math.min(page, pages));
        ...
        baseQuery += " LIMIT ? OFFSET ?";
        // Ejecutar consulta
        const [reviews] = await db.query(baseQuery, [
            producto_id,
            limit,
            offset,
        ]);
